...
void _print_place(struct robot* __robots, int __number_of_robots, int __row, int __col){
    for (int robotIdx = 0; robotIdx < __number_of_robots; robotIdx++){
        ...
        if (__robot->row == __row && __robot->col == __col)  //W에 있으면 출력
    ...
    for (int row = 0; row < 6; row++){
        for (int col = 0; col < 6; col++){
            if (map_draw_default[row][col] == 'A' ||
                ...
                for (int robotIdx = 0; robotIdx < __number_of_robots; robotIdx++){
                    struct robot* __robot = &__robots[robotIdx];
                    if (__robot->col == col && __robot->row == row){ //해당 위치에 해당되면
                        if (__robot->current_payload > 0) //물건을 들고 있으면
                            printf("%sM%d ", __robot->name, __robot->current_payload); //그곳에 로봇과 들고 있는 화물 번호 출력
                        else //물건을 들고 있지 않으면
                            printf("%s   ", __robot->name); //로봇 이름만 출력
                        isFound = 1;
                    }
                }
